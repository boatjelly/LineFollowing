...
const unsigned char LWHL = 240;  // Turns left (controls L wheel), use when right sensor detects black
const unsigned char RWHL = 220;  // Turns right (controls R wheel), use when left detects black
...
ISR(ADC_vect) {
  ...
  switch (sensor) {
    case 0:
      ...
      if ((leftyLoosey < BLK) || (rightyTighty >= BLK)) {
        OCR0B = LWHL;
      } else {
        OCR0B = 0;
      }
    ...
    case 2:
      ...
      if ((rightyTighty < BLK) || (leftyLoosey >= BLK)) {
        OCR2A = RWHL;
      } else {
        OCR2A = 0;
      }
    ...
    case 3:
      // Handle edge cases
      // Nothing here intentionally.. Reserved for when edge cases are found
